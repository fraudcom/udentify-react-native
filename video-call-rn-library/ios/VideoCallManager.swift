...
public class VideoCallManager: NSObject {
...
#if canImport(UdentifyCommons)
        DispatchQueue.main.async {
            let hadActiveCall = VideoCallCameraView.hasActiveInstance
            VideoCallCameraView.endActiveCall()
            VideoCallScreenLock.releaseAll()
            let resultMap: [String: Any] = [
                "success": hadActiveCall,
                "status": "disconnected"
            ]
            resolve(resultMap)
        }
...
#if canImport(UdentifyCommons)
    // Shared by the JS-invoked cancelVideoCall bridge method: posts the
    // cancellation to the server via VideoCallCameraView's currentInstance.
    ...
    // while the call is still ringing/connecting, or taps the JS-rendered End Call
    // button). Requires an active call, since it relies on VideoCallCameraView's
...
class VideoCallOperatorImpl: VCCameraControllerDelegate {
    ...
    // see INTERNALS.md "iOS coarse call status (VideoCallManager.statusString)".
