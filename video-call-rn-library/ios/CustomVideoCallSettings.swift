...
/// `.clear` and knows nothing about per-element visibility, so both have to
...
struct VideoCallStyleOverrides {
  ...
  /// buttons centred as a group (each shifted half a gap off centre) has to
  /// travel alongside the settings object.
