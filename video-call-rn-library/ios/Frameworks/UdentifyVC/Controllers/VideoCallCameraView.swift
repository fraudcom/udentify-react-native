...
public class VideoCallCameraView: UIView {
    ...
    private var isCameraSwitchInProgress = false
    ...
    @objc private func handleAppDidEnterBackground() {
        Task { @MainActor [weak self] in
            ...
            do {
                // NOTE: do NOT try AudioEngineAvailability(isInputAvailable: false,
                // isOutputAvailable: true) here to keep playout draining the jitter buffer -
                // the asymmetric combination SIGSEGVs inside the ADM on LiveKit 2.13.0.
                // Only .default and .none are safe.
    ...
    private func startIfReady() {
        // `isDismissalInProgress` is never cleared, so it doubles as "this view's
        // call is over": teardown() resets `hasStartedConnecting`, so without it an
        // ended call restarts itself on the next attach or `config` update.
        guard !isDismissalInProgress, !hasStartedConnecting, window != nil,
    ...
    // INTERNALS-REACT.md "Waiting-screen label style on iOS (applyInstructionLabelStyle)".
    @MainActor
    ...
    private func setParticipants() async {
        ...
        if room.remoteParticipants.count > 0 {
            ...
            // see INTERNALS-REACT.md "iOS agent-reconnect handling (VideoCallCameraView)".
    ...
    @objc private func cameraSwitchTapped() {
        let willSwitchToBack = !isSwitchedToRearCamera
        let type = willSwitchToBack ? "CLIENT_CAMERA_SWITCH_BACK_RESPONSE" : "CLIENT_CAMERA_SWITCH_FRONT_RESPONSE"
        performCameraSwitch(toBack: willSwitchToBack, responseType: type)
    }
    ...
    public func room(_ room: Room, participantDidConnect participant: RemoteParticipant) {
        ...
        Task { @MainActor [weak self] in
            ...
            if participantMetadata == "agent" {
                ...
                // reconnects; see INTERNALS-REACT.md "iOS agent-reconnect handling
                // (VideoCallCameraView)".
...
private extension VideoCallCameraView {
    ...
    // Both the in-call button and the agent's signal land here, so a switch from
    // either side is serialised against the other and answered with the same
    // CLIENT_CAMERA_SWITCH_*_RESPONSE. Overlapping switches are rejected rather
    // than queued: LiveKit's capture start/stop is reference-counted, and two
    // restarts interleaving leave that count unbalanced, after which every later
    // switch silently no-ops.
    private func performCameraSwitch(toBack shouldSwitchToBack: Bool, responseType: String) {
        guard !isCameraSwitchInProgress else {
            sendSignalResponse(type: responseType, success: false, message: "Switch already in progress")
            return
        }
        guard isSwitchedToRearCamera != shouldSwitchToBack else {
            sendSignalResponse(type: responseType, success: true, message: "Already using \(shouldSwitchToBack ? "back" : "front") camera")
            return
        }
        ...
        Task { @MainActor [weak self] in
            guard let self = self else { return }
            defer { self.isCameraSwitchInProgress = false }
            ...
            do {
                ...
                // Not in the upstream SDK: the agent can drive the camera over the
                // data channel, and Android repaints for remote switches as well.
                self.updateCameraSwitchButtonAppearance()
                VCSettings.logger?.info(logMessage: "Camera switched to \(shouldSwitchToBack ? "back" : "front")", logPeriod: .onProcess)
    ...
    func handleRemoteCameraSignal(shouldSwitchToBack: Bool) {
        ...
        performCameraSwitch(toBack: shouldSwitchToBack, responseType: responseType)
    }
