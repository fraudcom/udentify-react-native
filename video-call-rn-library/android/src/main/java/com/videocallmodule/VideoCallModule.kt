...
class VideoCallModule(reactContext: ReactApplicationContext) :
    ...
    // SDK's own in-call button. VideoCallFragment resolves the active call
    ...
    fun setMicrophoneEnabled(enabled: Boolean, promise: Promise) {
        UiThreadUtil.runOnUiThread {
            try {
                ...
                promise.resolve(VideoCallFragment.setMicrophoneEnabled(enabled))
            } catch (e: Throwable) {
    ...
    fun isMicrophoneEnabled(promise: Promise) {
        UiThreadUtil.runOnUiThread {
            try {
                promise.resolve(VideoCallFragment.isMicrophoneEnabled())
            } catch (e: Throwable) {
    ...
    // Deprecated alias for endVideoCall, kept for existing call sites.
    @ReactMethod
