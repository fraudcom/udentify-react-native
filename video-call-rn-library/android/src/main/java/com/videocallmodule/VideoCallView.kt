...
class VideoCallView(context: Context) : FrameLayout(context) {
    ...
    fun endCall(onEnded: ((Boolean, String?) -> Unit)? = null) {
        ...
        op.cancelVideoCall { success, error ->
            UiThreadUtil.runOnUiThread {
                // Inline rather than endCallLocally(), which would post behind
                // this block and report the call ended before it was. `op` rather
                // than `operator`: the SDK dismisses its own controller once the
                // cancel settles, which may already have cleared the field.
                op.endVideoCall()
                teardown()
    ...
    // so callers needing it to have finished inline it, as endCall does.
    fun endCallLocally() {
        UiThreadUtil.runOnUiThread {
