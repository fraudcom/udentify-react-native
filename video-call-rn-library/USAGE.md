...
Native SDK `26.3.0914` on both platforms.
...
A call with no camera or microphone access no longer fails somewhere obscure.
The SDK refuses to start — no access token is fetched, no room is joined — and
...
specific type. Previously these arrived as `UNKNOWN`, which made a real
failure indistinguishable from an unrecognised one.

### Audio stops when the app leaves the screen

An app configured to keep running in the background previously kept capturing
the microphone and playing the operator's voice while the user was somewhere
else entirely.

- Leaving the app now mutes the microphone through the normal signal path, so
...
|---|---|---|
| `endVideoCall()` | `VideoCallResult` | Ends the active call. `success` is `false` if no call was active. Safe to call when nothing is running. |
| `cancelVideoCall()` | `VideoCallResult` | Cancels the transaction server-side — for backing out while still connecting. Rejects with `NO_ACTIVE_CALL` if no call is mounted. |
...
is safe. Call `endVideoCall()` explicitly when you need to know it finished.
