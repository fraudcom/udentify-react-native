...
export function subscribeToVideoCallEvents(
  ...
  if (handlers.onRepeatStarted) {
    subscriptions.push(
      emitter.addListener('VideoCall_onRepeatStarted', (data: any) => {
        handlers.onRepeatStarted?.({role: data?.role});
      })
    );
  }
  if (handlers.onRepeatResult) {
    subscriptions.push(
      emitter.addListener('VideoCall_onRepeatResult', (data: any) => {
        handlers.onRepeatResult?.({role: data?.role});
      })
    );
  }
  if (handlers.onRepeatEnded) {
    subscriptions.push(
      emitter.addListener('VideoCall_onRepeatEnded', (data: any) => {
        handlers.onRepeatEnded?.({
          role: data?.role,
