...
Pod::Spec.new do |s|
  ...
  # PrivacyInfo.xcprivacy is deliberately not declared as a resource here, to avoid
  # conflicts with React Native's automatic privacy-manifest aggregation.
  #
  # IMPORTANT: do not add the *.lproj folders to exclude_files "to avoid double-bundling
  # with resource_bundles" (as a previous version of this file did). CocoaPods applies
  # exclude_files globally across resources AND resource_bundles alike (see
  # Sandbox::FileAccessor#resource_bundles in the cocoapods gem), so excluding the .lproj
  # files here silently empties out the VideoCallLibraryResources bundle too, breaking all
  # of this pod's iOS localization at runtime.
  s.exclude_files = [
