...
video-call-rn-library-26.3.914.tgz   # Frameworks/UdentifyVC (vendored Swift source) + LiveKitClient
```
...
npm install ./video-call-rn-library-26.3.914.tgz # For agent-assisted video call
```
...
- **Current Version**: 26.3.814 (`video-call-rn-library` is ahead at `26.3.914` — see its own Version History entry below)
- **Distribution**: 6 TGZ packages (1 core + 5 feature libraries)
- **Compatibility**: React Native 0.73.0+ (libraries), 0.76.0+ (test-app)
- **Native SDK Version**: Udentify SDK 26.3.0814 (`video-call-rn-library`: iOS 26.3.0914, Android 26.3.0914)
...
### `video-call-rn-library` 26.3.914 (Current for this package — Native SDK 26.3.0914, both platforms)
...
- ✅ New `VideoCallErrorType.SDK`; native SDK errors previously all arrived as `UNKNOWN`, which made a real failure indistinguishable from an unrecognised one
- ✅ Audio no longer keeps playing when the app leaves the screen: the microphone is muted through the normal signal path (so the operator sees an indicator) and the audio engine is stopped, both restored on return with the pre-existing mute state replayed
- ✅ Android: `requestPermissions()` now settles — it previously showed the dialog but dropped the result, leaving every `await` pending forever
- ✅ Android: `onVideoCallEnded` no longer reports `success: true` for a call that failed
- ✅ Android: the SDK now restores the user's intended mute state after a reconnect, which LiveKit itself does not
...
- ✅ iOS: the operator toggling their camera mid-call no longer restarts the call-initiation countdown
...
- ✅ iOS: screen no longer auto-locks during an active call; a muted remote camera shows a black overlay instead of a frozen frame (see `USAGE.md` section 7)
- ✅ Android: fixed default-interface-method routing so the new vendor callbacks aren't silently dropped by this library's reflection proxy
