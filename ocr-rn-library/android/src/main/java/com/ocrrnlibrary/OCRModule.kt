...
class OCRModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    // Opts this app into in-call OCR re-captures (the video call "repeat" flow)
    // and supplies the credentials one would use. Pass null to opt back out.
    // Without this, the video call SDK reports "repeat handler not available" to
    // the operator, which is the default and a perfectly working state.
    @ReactMethod
    fun setRepeatConfig(config: ReadableMap?, promise: Promise) {
        try {
            OcrLiveKycRepeat.setConfig(config)
            promise.resolve(true)
        } catch (e: IllegalArgumentException) {
            promise.reject("INVALID_REPEAT_CONFIG", e.message, e)
        } catch (e: Throwable) {
            promise.reject("REPEAT_CONFIG_FAILED", "Failed to configure OCR repeat: ${e.message}", e)
        }
    }
