...
// Whether this app opts into the in-call LiveKYC repeat flow (OCR / Face / NFC
// re-captures driven by the operator). true is the LiveKYC configuration;
// false is what an existing video-call integration looks like, and is what the
// backward-compatibility pass tests.
const ENABLE_LIVEKYC_REPEATS = true;
...
import VideoCallActiveScreen from './src/components/VideoCallActiveScreen';
import {VideoCall, type VideoCallCredentials} from 'video-call-rn-library';
// In-call "repeat" flow: the operator can ask mid-call for an OCR/Face/NFC
// re-capture. Each capture package is opted out until its setRepeatConfig is
// called, so without these the video call SDK answers the operator with
// "repeat handler not available" and nothing opens on the phone.
...
function App(): React.JSX.Element {
  ...
  const [activeCallCredentials, setActiveCallCredentials] = useState<VideoCallCredentials | null>(null);
  ...
  // Ask for every runtime permission the app's flows need, once, at startup.
  //
  // Done here rather than lazily per flow because iOS shows each system prompt
  // only once per install: whoever asks first owns the single moment the user
  // can grant it, and the Udentify SDKs deliberately check permissions without
  // ever requesting them. Asking up front also means a flow never dies halfway
  // through on a permission the user could have granted at launch.
  //
  // VideoCall.requestPermissions() owns the list on both platforms, so the app
  // never has to keep its own copy of which permissions the SDKs need.
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        console.error('App - startup permission request failed:', error);
      }
    })();
  ...
  // Registers (config != null) or clears (null) the in-call repeat handlers for
  // all three capture packages. Android-only inside each package - they return
  // false on iOS rather than throwing - and each is settled independently so one
  // package failing to register does not take the other two down with it.
  async function configureRepeatHandlers(
    credentials: VideoCallCredentials | null,
  ) {
    const common = credentials && {
    ...
    const results = await Promise.allSettled([
      setOcrRepeatConfig(
        common && {...common, country: 'TUR', documentType: 'ID_CARD'},
      ),
      setFaceRepeatConfig(
        common && {
          ...common,
          method: 'ActiveLiveness',
          // false = REGISTER a new face, true = AUTHENTICATE an existing one.
          // Was `true`, which failed with ERR_FACE_USER_ID_NOT_ENROLLED: the
          // capture itself succeeded and the server rejected it because this
          // transaction's user has never been enrolled. Each run uses a fresh
          // transaction, so register is the correct mode for this harness.
          isAuthenticate: false,
          // The SDK draws a full-screen udentifyface_bg_color image
          // (gesture_presentation_bg) over its own camera preview and takes
          // that image's alpha from here. Its default is 1, fully opaque,
          // which is why the active-liveness step showed a solid purple
          // screen instead of the customer's face. 0 lets the camera through.
          activeLivenessOpacity: 0,
        },
      ),
      setNfcRepeatConfig(common),
    ]);
    results.forEach((r, i) => {
      const role = ['OCR', 'Face', 'NFC'][i];
      if (r.status === 'rejected') {
        console.warn(`App - ${role} repeat config failed:`, r.reason);
      } else {
        console.log(`App - ${role} repeat ${credentials ? 'enabled' : 'cleared'}:`, r.value);
      }
    });
  }

  // Handle starting video call from VideoCallStartScreen: assembles
  // credentials as before, then mounts <VideoCallView> (via the
  // 'videoCallActive' screen) instead of calling the deprecated imperative
  // startVideoCall() - the embedded view starts connecting once mounted with
  // valid credentials props.
  async function handleStartVideoCall() {
    ...
    try {
      ...
      if (!permissions.hasCameraPermission || !permissions.hasRecordAudioPermission) {
        // Continues even when this is refused. The SDK gates the call itself and
        // reports the precise reason through onError (ERR_CAMERA_PERMISSION_* /
        // ERR_MICROPHONE_PERMISSION_*), which is more actionable than a generic
        // alert here - and blocking the call early would hide it.
        await VideoCall.requestPermissions();
      }
      ...
      // Opt in before the view mounts: the registration has to be in place by
      // the time the operator asks, and these share the call's own serverURL /
      // userID / transactionID so a repeat lands on the same transaction.
      //
      // Set ENABLE_LIVEKYC_REPEATS = false to run the call the way an existing
      // customer who has never adopted LiveKYC does - no repeat handlers
      // registered at all. That is the backward-compatibility path, and several
      // wrapper behaviours are gated on this registration, so it is the only way
      // to exercise them being switched off.
      await configureRepeatHandlers(ENABLE_LIVEKYC_REPEATS ? credentials : null);

      setActiveCallCredentials(credentials);
      setIsVideoCallLoading(false);
      setScreen('videoCallActive');
    } catch (error) {
  ...
  if (screen === 'videoCallActive' && activeCallCredentials) {
    return (
      <VideoCallActiveScreen
        credentials={activeCallCredentials}
        config={videoCallConfig}
        onEnded={(success) => {
          // Opt back out: the credentials are transaction-scoped, so leaving
          // them registered would point a later call's repeat at a finished
          // transaction.
          configureRepeatHandlers(null);
          setActiveCallCredentials(null);
          setScreen(success ? 'videoCallSuccess' : 'videoCallStart');
        }}
        onDismissed={() => {
          configureRepeatHandlers(null);
          setActiveCallCredentials(null);
