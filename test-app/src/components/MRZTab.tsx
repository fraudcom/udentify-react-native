...
interface MRZTabProps {
  ...
  onSwitchToNFC?: () => void;
  onStartNfcFlow?: (documentNumber?: string, dateOfBirth?: string, dateOfExpiration?: string) => void;
}

const MRZTab: React.FC<MRZTabProps> = ({ onMrzDataExtracted, onSwitchToNFC, onStartNfcFlow }) => {
  // State variables
  ...
  const useForNfc = (): void => {
    if (mrzResult && mrzResult.success) {
      if (onStartNfcFlow) {
        // Preferred path: hand the extracted MRZ data straight to the NFC
        // read flow instead of just switching tabs.
        onStartNfcFlow(documentNumber ?? undefined, dateOfBirth ?? undefined, dateOfExpiration ?? undefined);
        return;
      }

      // Fallback: just switch to the NFC tab if that's all the host provides.
      if (onSwitchToNFC) {
        onSwitchToNFC();
      }
