...
const NFCTab: React.FC<NFCTabProps> = ({ mrzData }) => {
  ...
  // Subscribe to native NFC progress events
  useEffect(() => {
  ...
  async function requestPermissions(): Promise<boolean> {
    if (Platform.OS === 'android') {
      try {
        ...
        console.warn('NFCTab - Permission request error:', error);
        return false;
      }
    }
    return true;
  ...
  async function getTransactionID(): Promise<string> {
    ...
    try {
      ...
      console.warn('NFCTab - ❌ Failed to get transaction ID:', error);
  ...
  async function checkNFCStatus() {
    try {
      ...
      console.warn('NFCTab - NFC Status Error:', error);
  ...
  async function testPassportReading() {
    try {
      ...
      console.warn('NFCTab - NFC Passport Reading Error:', error);
      setIsReading(false);
      setProgress(0);
  ...
  async function testCancelReading() {
    try {
      ...
      console.warn('NFCTab - Cancel NFC Reading Error:', error);
  ...
  async function testGetNFCLocation() {
    try {
      ...
      console.warn('NFCTab - NFC Location Error:', error);
