...
const OCRResultCard: React.FC<OCRResultCardProps> = ({ result }) => {
  const formatDate = (timestamp: number) => {
    // iOS emits seconds (Date().timeIntervalSince1970); Android emits
    // milliseconds (System.currentTimeMillis()). Treat anything already
    // millisecond-scale (13 digits, i.e. > year-2001-in-seconds) as-is.
        ...
        {result.success && result.extractedData && (
          <View style={styles.section}>
            ...
            <View style={styles.subsection}>
              ...
              {/* 26.1.3: native-script fields (empty unless the document carries native-script data) */}
              {renderField('nativeFirstName', result.extractedData.nativeFirstName)}
              {renderField('nativeLastName', result.extractedData.nativeLastName)}
              {renderField('nativeGender', result.extractedData.nativeGender)}
            </View>
