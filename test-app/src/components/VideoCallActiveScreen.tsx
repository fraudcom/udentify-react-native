...
// The MIC ON / MIC OFF overlay buttons are React-level test scaffolding for
// exercising setMicrophoneEnabled() by hand. They are not part of the shipped
// surface - the SDK's own bottom control row is - so they stay hidden unless a
// developer explicitly wants them.
const SHOW_MIC_TEST_BUTTONS = false;
...
export default function VideoCallActiveScreen({
  ...
  // The SDK's waiting screen owns the display until the operator joins and the
  // call actually starts; the app's own controls should not sit on top of it.
  // `connected` is the SDK's call-started marker - it follows the
  // initiating/tokenFetching/tokenFetched/connecting sequence and lands after
  // the operator's video has attached, so it is the right moment to reveal
  // them. The SDK reports it more than once, which setting a flag absorbs.
  const [callStarted, setCallStarted] = useState(false);
  // An in-call re-capture (OCR / Face / NFC) takes over the screen with its own
  // capture UI. The app's controls must get out of the way while it runs: they
  // would otherwise float over the capture UI and their touch targets can sit on
  // top of its buttons.
  const [repeatRunning, setRepeatRunning] = useState(false);
  // Kept in sync with repeatRunning for any handler that needs the live value
  // rather than the one captured when it was registered - a BackHandler would,
  // since it closes over its scope once. See the note above activeLabel.
  const repeatRunningRef = useRef(false);
  ...
  // No BackHandler is bound here, on purpose (2026-09-23).
  //
  // With no JS handler, React Native falls through to the Activity's own back
  // dispatcher, which is what the capture SDK expects: its "retake" calls
  // FragmentActivity.onBackPressed() and relies on the OnBackPressedCallback it
  // registered there. Any handler that consumes the press - as the previous one
  // did, by hanging up - silently disables retake, because
  // ReactActivity.onBackPressed() gives JS first refusal and only reaches the
  // dispatcher when JS declines.
  //
  // The consequence to know: outside a repeat nothing claims the press either,
  // so back reaches the Activity default and leaves this screen with the call
  // still running. Hanging up on back can come back, but it has to decline
  // while a repeat is in progress - see repeatRunningRef, kept for that.
  ...
  return (
    // box-none: under full-screen hosting the SDK's call view sits behind this
    // React tree, so anything here that claims touches steals them from the
    // SDK's own controls. Only the actual buttons below should be touchable.
    <View style={styles.container} pointerEvents="box-none">
      ...
      <VideoCallView
        ...
        config={config}
        onRepeatStarted={event => {
          console.log('VideoCallActiveScreen - repeat started:', event.role);
          repeatRunningRef.current = true;
          setRepeatRunning(true);
        }}
        onRepeatResult={event => {
          console.log('VideoCallActiveScreen - repeat result:', event.role);
        }}
        // Restores the controls on onRepeatEnded, not onRepeatResult: a repeat
        // the operator cancels with "return to call" never produces a result,
        // and leaving the controls hidden would strand the user in a live call
        // with no way to hang up.
        onRepeatEnded={event => {
          console.log('VideoCallActiveScreen - repeat ended:', event.role);
          repeatRunningRef.current = false;
      ...
      <SafeAreaView
        ...
        {/* Drives setMicrophoneEnabled() from inside an active call, which is the
            ...
            React-level and test-only - they are not part of what ships to a
            customer, and they sit over the end-call button where they get in the
            way of testing the SDK's own controls. Hidden for now; flip
            SHOW_MIC_TEST_BUTTONS to bring them back. */}
        <View style={[styles.micTestRow, SHOW_MIC_TEST_BUTTONS ? null : styles.hidden]}
              pointerEvents={SHOW_MIC_TEST_BUTTONS ? 'auto' : 'none'}>
          <TouchableOpacity
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    // Transparent, not black: when the repeat flow is configured the SDK hosts
    // its call view in the activity's content view, behind the React root. An
    // opaque background here would hide the call - the SDK paints its own
    // backdrop. Under inline hosting the VideoCallView covers this anyway, so
    // transparency costs nothing there.
