...
const MRZTestPage: React.FC<MRZTestPageProps> = ({ onMrzDataExtracted, onSwitchToNFC }) => {
  ...
  const startMrzScanning = async (): Promise<void> => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Add to test results
        addTestResult('Camera Scan', 'Success', true, duration);
      } else {
        const errorMsg = result.errorMessage || 'Unknown error';
        setStatus(`MRZ scan failed: ${errorMsg} ❌`);
        addTestResult('Camera Scan', errorMsg, false, duration);
      }
    } catch (error) {
      const duration = Date.now() - scanStartTime;
      setScanDuration(duration);
      ...
      setIsScanning(false);
      setScanProgress(0);
      addTestResult('Camera Scan', `${error}`, false, duration);
    }
  };
  ...
  const processImageFromBase64 = async (): Promise<void> => {
    ...
    try {
      ...
      if (result.success) {
        ...
        addTestResult('Image Processing', 'Success', true, duration);
      } else {
        const errorMsg = result.errorMessage || 'Unknown error';
        setStatus(`MRZ image processing failed: ${errorMsg} ❌`);
        addTestResult('Image Processing', errorMsg, false, duration);
      }
    } catch (error) {
      const duration = Date.now() - startTime;
      setStatus(`MRZ image processing failed: ${error} ❌`);
      setIsImageProcessing(false);
      addTestResult('Image Processing', `${error}`, false, duration);
    }
  };
  ...
  const cancelScanning = async (): Promise<void> => {
    try {
      ...
      setIsScanning(false);
      setScanProgress(0);
      addTestResult('Cancel Scan', 'Cancelled by user', false, duration);
    } catch (error) {
