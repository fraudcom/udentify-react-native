...
// OptionPicker takes {value, label} pairs; TIMEOUTS is a value-only map, so the
// labels are spelled out here rather than showing a bare number of seconds.
type IdleTimeout = (typeof TIMEOUTS)[keyof typeof TIMEOUTS];

const TIMEOUT_OPTIONS: {value: IdleTimeout; label: string}[] = [
  {value: TIMEOUTS.SHORT, label: '15 seconds'},
  {value: TIMEOUTS.DEFAULT, label: '30 seconds'},
  {value: TIMEOUTS.MEDIUM, label: '60 seconds'},
  {value: TIMEOUTS.LONG, label: '120 seconds'},
];
...
// Not named isMicrophoneEnabled: the component below holds state under that
// name, which would shadow this helper inside it.
const readMicrophoneState = async () => {
  if (!VideoCallModule) throw new Error('VideoCallModule not available');
...
const cancelVideoCall = async () => {
  if (!VideoCallModule) throw new Error('VideoCallModule not available');
  return await VideoCallModule.cancelVideoCall();
};
...
const VideoCallTab: React.FC = () => {
  ...
  const [idleTimeout, setIdleTimeout] = useState<IdleTimeout>(TIMEOUTS.DEFAULT);
  ...
  const setupEventListeners = () => {
    ...
    // The only thing that moves the mic indicator - the SDK also mutes and
    // unmutes on its own (backgrounding, audio focus, an incoming phone call),
    // so tracking what the buttons requested would drift out of sync.
    const microphoneStateListener = eventEmitter.addListener('VideoCall_onMicrophoneStateChanged', (data) => {
      setIsMicrophoneEnabled(!!data?.enabled);
      setStatus(`Microphone ${data?.enabled ? 'enabled' : 'disabled'}`);
    });
    ...
    return () => {
      ...
      microphoneStateListener.remove();
    };
  };
  ...
  const cancelVideoCallHandler = async () => {
    if (!isInCall) return;

    setStatus('Cancelling video call...');

    try {
      const result: VideoCallResult = await cancelVideoCall();

      setIsInCall(false);
      setCurrentStatus(VIDEO_CALL_STATUS.DISCONNECTED);

      if (result.success) {
        setStatus('Video call cancelled successfully');
      } else {
        setStatus(`Failed to cancel video call: ${result.error?.message || 'Unknown error'}`);
      }
    } catch (error) {
      setStatus(`Failed to cancel video call: ${error}`);
      setIsInCall(false);
    }
  };
  ...
  // Requests only: onMicrophoneStateChanged is what actually moves
  // isMicrophoneEnabled, so this never sets it itself.
  const setMicrophoneEnabledHandler = async (enabled: boolean) => {
    try {
      const accepted = await setMicrophoneEnabled(enabled);
      if (!accepted) {
        setStatus('Microphone request ignored: no active call');
      }
    } catch (error) {
      setStatus(`Failed to set microphone state: ${error}`);
    }
  };

  const readMicrophoneStateHandler = async () => {
    try {
      const enabled = await readMicrophoneState();
      setStatus(`Microphone is currently ${enabled ? 'on' : 'off'}`);
    } catch (error) {
      setStatus(`Failed to read microphone state: ${error}`);
    }
  };
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <OptionPicker
          ...
          onValueChange={value => setIdleTimeout(value as IdleTimeout)}
        />
      ...
      <View style={styles.card}>
        ...
        <TouchableOpacity
          style={[styles.button, styles.fullWidthButton, styles.cancelButton, isInCall ? {} : styles.disabledButton]}
          onPress={cancelVideoCallHandler}
          disabled={!isInCall}
        >
          <Text style={styles.buttonText}>Cancel Video Call</Text>
        </TouchableOpacity>
        ...
        <View style={styles.buttonRow}>
          <TouchableOpacity
            ...
            onPress={() => setMicrophoneEnabledHandler(false)}
            disabled={!isInCall}
          >
          ...
          <TouchableOpacity
            style={[styles.button, isInCall ? {} : styles.disabledButton]}
            onPress={() => setMicrophoneEnabledHandler(true)}
            disabled={!isInCall}
          >
        ...
        <View style={styles.buttonRow}>
          <TouchableOpacity
            style={[styles.button, isInCall ? {} : styles.disabledButton]}
            onPress={readMicrophoneStateHandler}
            disabled={!isInCall}
          >
            <Text style={styles.buttonText}>Read Mic State</Text>
          </TouchableOpacity>
        </View>
      </View>
