...
export const videoCallConfig: VideoCallConfig = {
  ...
  muteButtonStyle: {
    ...
    mutedColor: '#FF0000',
    // Green rather than the SDKs' white, and kept that way deliberately.
    //
    // It differs from BOTH fallbacks - the Android SDK's own
    // @color/udentify_vc_button_mic_on_color (#FFFFFF) and this app's override
    // of that same resource (also #FFFFFF) - so the colour on screen can only
    // have come from this config. That makes it a standing check that the
    // styling path still works: white here means VideoCallStyleApplier stopped
    // applying the tint, or the SDK's own repaint won.
    //
    // Verified 2026-09-23: stays green across local taps and remote mutes, both
    // of which make the SDK repaint the button with its own colour. The tint is
    // re-applied from onMicrophoneStateChanged rather than a polling timer.
    unmutedColor: '#00FF00',
