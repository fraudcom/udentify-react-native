...
try {
  ...
  console.warn('❌ MRZLibrary - Failed to load module:', error);
...
export interface MrzUICustomization {
  ...
  /** Header title shown at the top of the scan screen (design 3.2) */
  headerTitle?: string;
  /** Auto capture indicator text shown at the bottom (design 3.2) */
...
export async function checkPermissions(): Promise<boolean> {
  ...
  try {
    ...
    console.warn('MRZModule - checkPermissions error:', error);
    return false;
  }
}
...
export async function requestPermissions(): Promise<string> {
  ...
  try {
    ...
    console.warn('MRZModule - requestPermissions error:', error);
    return 'denied';
  }
}
...
export async function startMrzCamera(
  ...
  try {
    ...
    console.warn('MRZModule - startMrzCamera error:', error);
    return {
      success: false,
...
export async function processMrzImage(
  ...
  try {
    ...
    console.warn('MRZModule - processMrzImage error:', error);
    return {
      success: false,
...
export async function cancelMrzScanning(): Promise<void> {
  ...
  try {
    ...
    console.warn('MRZModule - cancelMrzScanning error:', error);
    throw error;
  }
}
...
// MrzErrorType, MrzData, BACCredentials, MrzResult, MrzProgressCallback and
// MrzUICustomization are already exported at their declarations above. Naming
// them again here is a duplicate export, which TypeScript rejects (TS2484) in
// any project that typechecks against this package - and this package ships its
// raw `src` as its type entry point, so that is every consumer.

// Inline camera view (renders MRZ SDK camera inside the RN view tree so
// JS overlays can sit on top). Does not replace startMrzCamera modal API.
export {MrzCameraView} from './MrzCameraView';
export type {MrzCameraViewProps} from './MrzCameraView';
