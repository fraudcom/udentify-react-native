...
class NFCManagerSwift: NSObject {
  ...
  func startNFCReading(credentials: [String: Any], completion: @escaping ([String: Any]?, Error?) -> Void) {
    DispatchQueue.main.async {
      ...
      // isActiveAuthenticationEnabled was replaced by ChipAnticloneMode in the 26.3.0814 SDK;
      // map the legacy boolean to its documented equivalent (requireAA / none) to preserve behavior.
      let isActiveAuthEnabled = credentials["isActiveAuthenticationEnabled"] as? Bool ?? true
      let chipAnticloneMode: ChipAnticloneMode = isActiveAuthEnabled ? .requireAA : .none
      ...
      self.nfcReader = NFCReader(
        ...
        chipAnticloneMode: chipAnticloneMode,
      ...
      self.nfcReader?.read { [weak self] (passport, error, progress) in
        DispatchQueue.main.async {
          if let passport = passport {
            ...
            // Raw Passport SDK fields (debug)
            print("NFCManager - Raw Passport fields:")
            print("  documentType=\(String(describing: passport.documentType))")
            print("  documentNumber=\(String(describing: passport.documentNumber))")
            print("  documentCode=\(String(describing: passport.documentCode))")
            print("  documentExpiryDate=\(String(describing: passport.documentExpiryDate))")
            print("  personalNumber=\(String(describing: passport.personalNumber))")
            print("  firstName=\(String(describing: passport.firstName))")
            print("  lastName=\(String(describing: passport.lastName))")
            print("  dateOfBirth=\(String(describing: passport.dateOfBirth))")
            print("  gender=\(String(describing: passport.gender))")
            print("  state=\(String(describing: passport.state))")
            print("  nationality=\(String(describing: passport.nationality))")
            print("  birthPlace=\(passport.birthPlace)")
            print("  address=\(passport.address)")
            print("  phoneNumber=\(passport.phoneNumber)")
            print("  userID=\(passport.userID)")

            // Convert passport data to dictionary
            let passportData = self?.convertPassportToDict(passport, transactionID: transactionID) ?? [:]
            let loggable = passportData.filter { $0.key != "faceImage" && $0.key != "idImage" && $0.key != "signatureImage" }
            print("NFCManager - Mapped passport dictionary (image fields stripped): \(loggable)")
            ...
            NotificationCenter.default.post(
              name: NSNotification.Name("NFCProgress"),
              object: nil,
