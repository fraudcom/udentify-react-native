...
class NFCModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    fun startNFCReading(
        ...
        try {
            ...
            if (currentActivity !is AppCompatActivity) {
                promise.reject("NFC_ERROR", reactApplicationContext.getString(R.string.nfc_error_invalid_activity_message))
                return
            }
            ...
            if (documentNumber == null || dateOfBirth == null || expiryDate == null ||
                ...
                promise.reject("NFC_ERROR", reactApplicationContext.getString(R.string.nfc_error_missing_credentials_message))
                return
            }
            ...
            val chipAnticloneMode = if (isActiveAuthEnabled) ChipAnticloneMode.requireAA else ChipAnticloneMode.none
    ...
    fun cancelNFCReading(promise: Promise) {
        try {
            ...
            passportCompletion?.reject("NFC_CANCELLED", reactApplicationContext.getString(R.string.nfc_reading_cancelled_message))
    ...
    // NFCReaderActivityWrapper, which isn't a subclass and so can't reach
    ...
    fun emitNFCProgress(progress: Int) {
        Log.d(NAME, "NFCModule - Emitting onNFCProgress event: $progress%")
        val params = WritableNativeMap()
