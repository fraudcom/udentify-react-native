...
class NFCReaderActivityWrapper : NFCReaderActivity() {
    ...
    companion object {
        ...
        fun handleFailure(throwable: Throwable) {
            try {
                ...
                staticNFCModule?.emitNFCError(throwable.message ?: staticNFCModule?.getLocalizedString(R.string.nfc_error_unknown_message) ?: "Unknown NFC error")
        ...
        fun handleStateChange(nfcState: NFCState) {
            when (nfcState) {
                ...
                NFCState.DISABLED -> {
                    ...
                    staticNFCModule?.emitNFCError(staticNFCModule?.getLocalizedString(R.string.nfc_error_nfc_disabled_message) ?: "NFC is disabled on this device")
                }
                NFCState.ENABLING -> {
    ...
    override fun onFailure(throwable: Throwable) {
        ...
        // Emit error event
        staticNFCModule?.emitNFCError(throwable.message ?: getString(R.string.nfc_error_unknown_message))
    ...
    override fun onState(nfcState: NFCState) {
        ...
        when (nfcState) {
            ...
            NFCState.DISABLED -> {
                Log.w(TAG, "NFC is disabled")
                staticNFCModule?.emitNFCError(getString(R.string.nfc_error_nfc_disabled_message))
            }
            NFCState.ENABLING -> {
                Log.d(TAG, "NFC is enabling")
            }
    ...
    override fun onProgress(progress: Int) {
        ...
        staticNFCModule?.emitNFCProgress(progress)
    }
...
class InlineNFCReaderFragment : NFCReaderFragment() {
    ...
    override fun onProgress(progress: Int) {
        ...
        NFCReaderActivityWrapper.staticNFCModule?.emitNFCProgress(progress)
    }
