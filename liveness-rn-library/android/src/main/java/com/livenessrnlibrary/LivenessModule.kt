...
import io.udentify.android.face.activities.DemographicsEstimationListener
import io.udentify.android.face.activities.FaceListUserListenerHandler
import io.udentify.android.face.activities.FaceService
import io.udentify.android.face.model.DemographicsResult
...
class LivenessModule(reactContext: ReactApplicationContext) :
    ...
    // Opts this app into in-call face re-captures (the video call "repeat"
    // flow) and supplies the credentials one would use. Pass null to opt back
    // out. Without this, the video call SDK reports "repeat handler not
    // available" to the operator, which is the default and a working state.
    @ReactMethod
    fun setRepeatConfig(config: ReadableMap?, promise: Promise) {
        try {
            FaceLiveKycRepeat.setConfig(config)
            promise.resolve(true)
        } catch (e: IllegalArgumentException) {
            promise.reject("INVALID_REPEAT_CONFIG", e.message, e)
        } catch (e: Throwable) {
            promise.reject("REPEAT_CONFIG_FAILED", "Failed to configure the repeat: ${e.message}", e)
        }
    }
    ...
    private fun startFaceRecognition(credentials: ReadableMap, method: FaceRecognitionMethod, promise: Promise) {
        ...
        if (!hasRequiredPermissions()) {
            promise.reject("PERMISSIONS_NOT_GRANTED", reactApplicationContext.getString(R.string.liveness_permissions_not_granted))
            return
        }
        ...
        try {
            ...
            val result = Arguments.createMap().apply {
                ...
                putMap("faceIDMessage", Arguments.createMap().apply {
                    ...
                    putString("message", reactApplicationContext.getString(R.string.liveness_face_recognition_started))
                })
            }
    ...
    fun startActiveLiveness(credentials: ReadableMap, isAuthentication: Boolean = false, promise: Promise) {
        ...
        try {
            ...
            val result = Arguments.createMap().apply {
                ...
                putMap("faceIDMessage", Arguments.createMap().apply {
                    putBoolean("success", true)
                    putString("message", reactApplicationContext.getString(R.string.liveness_active_liveness_started))
                })
            }
            promise.resolve(result)
        } catch (e: Exception) {
    ...
    fun startHybridLiveness(credentials: ReadableMap, isAuthentication: Boolean, promise: Promise) {
        ...
        try {
            ...
            val result = Arguments.createMap().apply {
                ...
                putMap("faceIDMessage", Arguments.createMap().apply {
                    putBoolean("success", true)
                    putString("message", reactApplicationContext.getString(R.string.liveness_hybrid_liveness_started))
                })
            }
            promise.resolve(result)
        } catch (e: Exception) {
    ...
    fun startSelfieCapture(credentials: ReadableMap, promise: Promise) {
        ...
        try {
            ...
            val result = Arguments.createMap().apply {
                ...
                putMap("faceIDMessage", Arguments.createMap().apply {
                    putBoolean("success", true)
                    putString("message", reactApplicationContext.getString(R.string.liveness_selfie_capture_started))
                })
            }
            promise.resolve(result)
        } catch (e: Exception) {
    ...
    fun performFaceRecognitionWithSelfie(credentials: ReadableMap, base64Image: String, isAuthentication: Boolean, promise: Promise) {
        ...
        try {
            ...
            val result = Arguments.createMap().apply {
                ...
                putMap("faceIDMessage", Arguments.createMap().apply {
                    putBoolean("success", true)
                    putString("message", reactApplicationContext.getString(R.string.liveness_face_recognition_selfie_started))
                })
            }
            promise.resolve(result)
        } catch (e: Exception) {
    ...
    fun registerUserWithPhoto(credentials: ReadableMap, base64Image: String, promise: Promise) {
        try {
            ...
            val result = Arguments.createMap().apply {
                ...
                putMap("faceIDMessage", Arguments.createMap().apply {
                    putBoolean("success", true)
                    putString("message", reactApplicationContext.getString(R.string.liveness_registration_started))
                })
            }
            promise.resolve(result)
        } catch (e: Exception) {
    ...
    fun authenticateUserWithPhoto(credentials: ReadableMap, base64Image: String, promise: Promise) {
        try {
            ...
            val result = Arguments.createMap().apply {
                ...
                putMap("faceIDMessage", Arguments.createMap().apply {
                    putBoolean("success", true)
                    putString("message", reactApplicationContext.getString(R.string.liveness_authentication_started))
                })
            }
            promise.resolve(result)
        } catch (e: Exception) {
    ...
    /**
     * 26.1.3: Estimate demographics (age + gender) from a base64-encoded selfie image.
     */
    @ReactMethod
    fun estimateDemographics(
        serverURL: String,
        ...
        try {
            ...
            val faceService = FaceService(activity)
            faceService.estimateDemographics(serverURL, transactionID, imageBase64,
                object : DemographicsEstimationListener {
                    override fun onDemographicsResult(demographicsResult: DemographicsResult) {
                        val result = Arguments.createMap().apply {
                            putInt("estimatedAge", demographicsResult.estimatedAge)
                            putString("estimatedGender", demographicsResult.estimatedGender)
                        }
                        promise.resolve(result)
                    }

                    override fun onDemographicsError(errorMessage: String) {
                        promise.reject("DEMOGRAPHICS_ERROR", errorMessage)
                    }
                })
        } catch (e: Exception) {
            promise.reject("DEMOGRAPHICS_ERROR", "Failed to estimate demographics: ${e.message}")
        }
    }
    ...
    fun addUserToList(serverURL: String, transactionId: String, status: String, metadata: ReadableMap?, promise: Promise) {
        try {
            ...
            val isSDKAvailable = try {
                Class.forName("io.udentify.android.face.activities.FaceService")
                true
            ...
            if (!isSDKAvailable) {
                promise.reject("SDK_NOT_AVAILABLE", reactApplicationContext.getString(R.string.liveness_sdk_not_available))
                return
            }
    ...
    fun startFaceRecognitionIdentification(serverURL: String, transactionId: String, listName: String, logLevel: String?, promise: Promise) {
        ...
        try {
            // userID is unknown ahead of time for identification (the server matches against
            // listName instead); logLevel has no wiring point on this SDK's FaceCameraFragment
            // path (unlike iOS's IDCameraController.instantiate, which takes it directly).
            val parsedCredentials = FaceRecognizerCredentials(
                serverURL = serverURL,
                transactionID = transactionId,
                userID = java.util.Date().time.toString(),
                listName = listName
            )

            android.util.Log.i("LivenessModule", "🎯 Starting face recognition identification - List: $listName")

            faceRecognizerImpl = FaceRecognizerImpl(this)

            val success = faceRecognizerImpl?.startFaceRecognitionWithCamera(activity, parsedCredentials, FaceRecognitionMethod.IDENTIFICATION) ?: false
            ...
            promise.reject("IDENTIFICATION_FAILED", "Failed to start face recognition identification: ${e.message}")
        }
    }
    ...
    fun deleteUserFromList(serverURL: String, transactionId: String, listName: String, photoBase64: String, promise: Promise) {
        try {
            ...
            val faceService = FaceService(activity)
            val handler = FaceListUserListenerHandler()
                .onDeleteSuccess { listUser ->
                    val result = Arguments.createMap().apply {
                        putBoolean("success", true)
                        putString("message", reactApplicationContext.getString(R.string.liveness_user_deleted_from_list))
                        putString("userID", listUser?.userID ?: "")
                        putString("transactionID", listUser?.transactionID ?: transactionId)
                        putString("listName", listUser?.listName ?: listName)
                        putDouble("matchScore", listUser?.matchScore ?: 0.0)
                        putString("registrationTransactionID", listUser?.registrationTransactionID ?: "")
                    }
                    promise.resolve(result)
                }
                .onDeleteError { faceError ->
                    promise.reject("DELETE_USER_FROM_LIST_ERROR", faceError?.message ?: "Failed to delete user from list")
                }

            faceService.deleteUserFromList(serverURL, transactionId, listName, photoBase64, handler)
        } catch (e: Exception) {
            promise.reject("DELETE_USER_FROM_LIST_ERROR", "Failed to delete user from list: ${e.message}")
        }
    }
    ...
    private fun parseFaceRecognizerCredentials(credentials: ReadableMap): FaceRecognizerCredentials {
        ...
        return FaceRecognizerCredentials(
            ...
            activeLivenessOpacity =
                if (credentials.hasKey("activeLivenessOpacity"))
                    credentials.getDouble("activeLivenessOpacity").toFloat()
                else null,
    ...
    // Note: the JS API names the third parameter "status", but the real SDK's addUserToList only
    // accepts a listName + metadata (no separate status concept) — threaded through to listName
    // here, the only identifier string the native call accepts (matches iOS's addUserToList).
    private fun addUserToListWithSDK(
        ...
        val faceService = FaceService(activity)
        val handler = FaceListUserListenerHandler()
            .onAddSuccess { listResponseData ->
                val result = Arguments.createMap().apply {
                    putBoolean("success", true)
                    putMap("data", Arguments.createMap().apply {
                        listResponseData?.id?.let { putInt("id", it) }
                        listResponseData?.userId?.let { putInt("userId", it) }
                        listResponseData?.customerList?.let { customerList ->
                            putMap("customerList", Arguments.createMap().apply {
                                customerList.id?.let { putInt("id", it) }
                                putString("name", customerList.name ?: "")
                                putString("listRole", customerList.listRole ?: "")
                                putString("description", customerList.description ?: "")
                                putString("creationDate", customerList.creationDate ?: "")
                            })
                        }
                    })
                }
            ...
            .onAddError { faceError ->
                promise.reject("ADD_USER_TO_LIST_ERROR", faceError?.message ?: "Failed to add user to list")
            }

        faceService.addUserToList(serverURL, transactionId, status, metadata?.toHashMap(), handler)
    }

    // Exposes the (otherwise protected) React context's string resources to
    // FaceRecognizerImpl, which isn't a subclass and so can't reach
    // reactApplicationContext directly under Kotlin's visibility rules.
    fun getLocalizedString(resId: Int): String = reactApplicationContext.getString(resId)
