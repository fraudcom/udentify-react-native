...
export interface FaceRecognizerCredentials {
  ...
  /**
   * Opacity of the active-liveness gesture backdrop, `0`–`1`. **Android only.**
   * The SDK draws a full-screen image (`gesture_presentation_bg`) over its own
   * camera preview during an active-liveness capture and takes that image's
   * alpha from here. It defaults to **`1`** — fully opaque — so the customer
   */
...
// Demographics estimation result (26.1.3)
export interface DemographicsResult {
  /** Estimated age in years. */
  estimatedAge: number;
  /** Estimated gender (e.g. 'male' | 'female'). */
