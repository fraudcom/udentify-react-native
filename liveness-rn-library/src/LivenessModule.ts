...
if (isTurboModuleEnabled) {
  try {
    ...
    console.warn('LivenessModule - TurboModule failed, falling back to Legacy:', error);
...
class LivenessModule {
  ...
  // Demographics estimation (26.1.3)
  async estimateDemographics(
    ...
    if (!LivenessModuleNative) throw new Error('LivenessModule not available');
    return (await LivenessModuleNative.estimateDemographics(
      serverURL,
      transactionID,
      imageBase64
    )) as DemographicsResult;
  }
  ...
  async configureUISettings(settings: UISettings): Promise<boolean> {
    ...
    try {
      ...
      console.warn('LivenessModule - Failed to configure UI settings:', error);
      throw error;
    }
  }
  ...
  async setLocalization(languageCode: string, customStrings?: LocalizationStrings): Promise<void> {
    ...
    try {
      ...
      console.warn('LivenessModule - Failed to set localization:', error);
      throw error;
    }
  }
  ...
  async applyUIConfiguration(settings: UISettings): Promise<boolean> {
    ...
    try {
      ...
      console.warn('LivenessModule - Failed to apply UI configuration:', error);
      throw error;
    }
  }
